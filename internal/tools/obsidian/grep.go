...
func (g *grepRun) visit(ctx context.Context, entry fsx.WalkFile) (fsx.WalkAction, error) {
	...
	if g.resume != nil && !g.boundarySeen {
		...
		_ = file.Close()
		g.work.SourceEntriesValidated++
		if order < 0 {
			g.prefix = extendGrepPrefix(g.prefix, position, fingerprint)
			return fsx.WalkContinue, nil
		}
		g.boundarySeen = true
		expectedPrefix, _ := decodeDigest(g.resume.Prefix)
		if g.resume.Partial == nil {
			g.prefix = extendGrepPrefix(g.prefix, position, fingerprint)
			if !equalDigest(g.prefix[:], expectedPrefix) {
				return fsx.WalkStop, ErrCursorStale
			}
		...
		if encodeDigest(fingerprint[:]) != g.resume.Partial.Fingerprint {
			return fsx.WalkStop, ErrCursorStale
		}
		return g.scanFile(ctx, entry, g.resume.Partial)
	}

	if g.work.FilesScanned >= uint64(g.query.MaxFiles) {
		if g.lastFull == nil {
			return fsx.WalkStop, &fsx.Error{Code: fsx.CodeLimitExceeded}
		}
		g.stop = &grepStop{reason: CursorStopFileLimit, state: g.fullBoundaryState(*g.lastFull)}
		return fsx.WalkStop, nil
	}
	if g.pageBytes >= g.query.MaxBytes {
		if g.lastFull == nil {
			return fsx.WalkStop, &fsx.Error{Code: fsx.CodeLimitExceeded}
		}
		g.stop = &grepStop{reason: CursorStopByteLimit, state: g.fullBoundaryState(*g.lastFull)}
		return fsx.WalkStop, nil
	}
	return g.scanFile(ctx, entry, nil)
}

func (g *grepRun) scanFile(ctx context.Context, entry fsx.WalkFile, partial *grepPartialCursor) (fsx.WalkAction, error) {
	if g.work.FilesScanned >= uint64(g.query.MaxFiles) {
		if g.lastFull == nil {
			return fsx.WalkStop, &fsx.Error{Code: fsx.CodeLimitExceeded}
		}
	...
	g.work.FilesScanned++
	fingerprint := file.Fingerprint()
	// A resumed boundary is opened once during catalog-prefix validation and
	// again here for content scanning. Recheck the cursor fingerprint on this
	// exact content descriptor so a replacement between those opens cannot join
	// evidence from two source versions.
	if partial != nil && encodeDigest(fingerprint[:]) != partial.Fingerprint {
		return fsx.WalkStop, ErrCursorStale
	}
	startOffset, startLine := int64(0), 1
	resumeOffset := int64(0)
	if partial != nil {
		startOffset, startLine = partial.ContextOffset, partial.ContextLine
		resumeOffset = partial.ResumeOffset
		if startOffset > entry.Resolved.Size || resumeOffset > entry.Resolved.Size {
			return fsx.WalkStop, ErrCursorStale
		}
		if err := file.Seek(ctx, startOffset); err != nil {
			return fsx.WalkStop, ErrCursorStale
		}
	}
	...
	checkpoint := g.partialCheckpoint(entry.Position, fingerprint, startOffset, startLine, startOffset, startLine)
	advanced := false

	for {
		line, readErr := reader.next(ctx)
		g.pageBytes += reader.takeNewBytes()
		g.work.BytesScanned = uint64(g.pageBytes)
		if errors.Is(readErr, io.EOF) {
			for len(pending) > 0 {
				if stop, err := g.emit(ctx, pending[0]); err != nil || stop {
		...
		if errors.Is(readErr, errLineTooLarge) {
			return fsx.WalkStop, &fsx.Error{Code: fsx.CodeInputTooLarge}
		}
		if errors.Is(readErr, errLineBudget) {
			stateCheckpoint := checkpoint
			if len(pending) > 0 {
				stateCheckpoint = pending[0].retry
			}
			if !advanced {
				if g.lastFull != nil && g.fullBoundaryAdvances(*g.lastFull) {
					g.stop = &grepStop{reason: CursorStopByteLimit, state: g.fullBoundaryState(*g.lastFull)}
					return fsx.WalkStop, nil
				}
				return fsx.WalkStop, &fsx.Error{Code: fsx.CodeLimitExceeded}
			}
			if !g.checkpointAdvances(stateCheckpoint) {
				return fsx.WalkStop, &fsx.Error{Code: fsx.CodeLimitExceeded}
			}
			g.stop = &grepStop{reason: CursorStopByteLimit, state: stateCheckpoint.state()}
			return fsx.WalkStop, nil
		}
		if readErr != nil {
			return fsx.WalkStop, readErr
		}
		if !utf8.ValidString(line.text) {
			return fsx.WalkStop, errInvalidUTF8
		}
		...
		if len(indexes) > 0 {
			...
			if len(before) > 0 {
				retryContextOffset, retryContextLine = before[0].start, before[0].number
			}
		...
		before = appendGrepRing(before, line, g.query.ContextLines)
		contextOffset, contextLine := grepResumeContext(before[:max(len(before)-1, 0)], line, g.query.ContextLines)
		checkpoint = g.partialCheckpoint(entry.Position, fingerprint, line.end, line.number+1, contextOffset, contextLine)
		if g.query.ContextLines == 0 {
			for len(pending) > 0 && pending[0].remaining == 0 {
				if stop, err := g.emit(ctx, pending[0]); err != nil || stop {
					return g.stopAction(err)
				}
...
func (r *grepLineReader) next(ctx context.Context) (grepLine, error) {
	...
	line := make([]byte, 0, min(MaxGrepPhysicalLineBytes, 32*1024))
	for {
		if r.pendingPos < len(r.pending) {
			...
			if newline := bytes.IndexByte(fragment, '\n'); newline >= 0 {
				...
				if len(line) > MaxGrepPhysicalLineBytes {
		...
		if r.offset >= r.size {
			...
			r.line++
			return grepLine{number: lineNumber, start: start, end: r.offset, text: string(line)}, nil
		}
		if r.remaining <= 0 {
		...
		lineAllowance := int64(MaxGrepPhysicalLineBytes + 1 - len(line))
